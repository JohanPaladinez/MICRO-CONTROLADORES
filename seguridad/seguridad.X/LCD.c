...
// Definición de caracteres especiales (candado, check, X)
unsigned char customChars[3][8] = {
    // Carácter candado (índice 0)
    {
        0b00000,
        0b01110,
        0b01010,
    ...
    // Carácter check/marca de verificación (índice 1)
    {
        0b00000,
        0b00001,
        0b00011,
...
{
    // Cargar caracteres personalizados en la CGRAM
    for (unsigned char i = 0; i < 3; i++) {
        LCD_Command(0x40 + (i * 8)); // Comando para definir CGRAM para cada carácter
        for (unsigned char j = 0; j < 8; j++) {
            LCD_Char(customChars[i][j]); // Cargar cada byte del carácter
        }
    }
}
/*F**************************************************************************
...
void LCD_Custom_Char(unsigned char loc, unsigned char *charmap)
{
    if (loc < 8) {
        ...
        for (int i = 0; i < 8; i++) {
            LCD_Char(charmap[i]); // Write the character pattern to CGRAM
        }
    }
}
